...
def _write_case(
    *, config: DatasetGenerationConfig, case: GeneratedCase, molecule, root: Path
) -> None:
    ...
    write_indexed_geometry(
        input_file=config.hyperfine.file, output_file=data_dir / "HFC" / "geometry.xyz"
    )
    ...
    write_experiment(
        config=config, case=case, output_file=data_dir / "PARA" / "generated_shifts.csv"
    )
    _write_diamagnetic_csv(
        molecule=molecule, output_file=data_dir / "DIA" / "diamagnetic.csv"
    )
...
def _write_diamagnetic_csv(*, molecule, output_file: Path) -> None:
    ...
    write_csv_safe(
        pd.DataFrame(
            [
                {"atom_label": nucleus.label, "shift": nucleus.shift.dia}
